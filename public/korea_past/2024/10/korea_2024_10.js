...
async function loadKoreaFireData() {
  try {
    // 상대 경로로 JSON 파일 접근 (korea_past/2024/10/ 에서 루트의 data 폴더로)
    ...
    const october2024Data = fireData.filter(item => {
      ...
      const dataDate = new Date(dateField.split(" ")[0]); // "2024-10-13 11:02"에서 날짜 부분만 추출
      return dataDate >= new Date("2024-10-01") && dataDate <= new Date("2024-10-31");
    });

    console.log(`✅ 2024년 10월 화재 데이터 ${october2024Data.length}개 필터링됨`);
    
    if (october2024Data.length === 0) {
      console.warn("⚠️ 2024년 10월 데이터가 없습니다. 전체 데이터 구조를 확인합니다:");
      console.log("첫 번째 데이터 샘플:", fireData[0]);
      updateFireCount("2024년 10월 데이터 없음");
      return;
    }
  ...
  for (const path of alternatePaths) {
    try {
      ...
      if (res.ok) {
        ...
        console.log(`📅 2024년 10월 데이터: ${october2024Data.length}개`);
...
// 화재 데이터 시각화
function renderFireData(fireData, startDate = "2024-10-01", endDate = "2024-10-31") {
  ...
  fireData.forEach((item, index) => {
    ...
    const dataDateStr = fireStartTime.split(" ")[0]; // "2024-10-13 11:02"에서 "2024-10-13" 추출
...
function setupDateRangeFilter(fireData) {
  ...
  const onRangeChange = () => {
    ...
    if (sDate < new Date("2024-10-01") || eDate > new Date("2024-10-31")) {
      alert("2024년 10월 범위 내에서만 선택 가능합니다.");
      return;
    }
