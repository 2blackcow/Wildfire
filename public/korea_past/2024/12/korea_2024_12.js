...
async function loadKoreaFireData() {
  try {
    ...
    const november2024Data = fireData.filter(item => {
      ...
      return dataDate >= new Date("2024-12-01") && dataDate <= new Date("2024-12-31");
    });

    console.log(`✅ 2024년 12월 화재 데이터 ${november2024Data.length}개 필터링됨`);
    
    if (november2024Data.length === 0) {
      console.warn("⚠️ 2024년 12월 데이터가 없습니다. 전체 데이터 구조를 확인합니다:");
      console.log("첫 번째 데이터 샘플:", fireData[0]);
      updateFireCount("2024년 12월 데이터 없음");
      return;
    }
  ...
  for (const path of alternatePaths) {
    try {
      ...
      if (res.ok) {
        ...
        console.log(`📅 2024년 12월 데이터: ${november2024Data.length}개`);
...
// 화재 데이터 시각화
function renderFireData(fireData, startDate = "2024-12-01", endDate = "2024-12-31") {
...
function setupDateRangeFilter(fireData) {
  ...
  const onRangeChange = () => {
    ...
    if (sDate < new Date("2024-12-01") || eDate > new Date("2024-12-31")) {
      alert("2024년 12월 범위 내에서만 선택 가능합니다.");
      return;
    }
