...
async function loadKoreaFireData() {
  try {
    ...
    const march2025Data = fireData.filter(item => {
      ...
      return dataDate >= new Date("2025-04-01") && dataDate <= new Date("2025-04-30");
    });

    console.log(`✅ 2025년 4월 화재 데이터 ${march2025Data.length}개 필터링됨`);
    
    if (march2025Data.length === 0) {
      console.warn("⚠️ 2025년 4월 데이터가 없습니다. 전체 데이터 구조를 확인합니다:");
      console.log("첫 번째 데이터 샘플:", fireData[0]);
      updateFireCount("2025년 4월 데이터 없음");
      updateFireList([]);
      return;
    }
  ...
  for (const path of alternatePaths) {
    try {
      ...
      if (res.ok) {
        ...
        console.log(`📅 2025년 4월 데이터: ${march2025Data.length}개`);
...
// 화재 데이터 시각화
function renderFireData(fireData, startDate = "2025-04-01", endDate = "2025-04-30") {
...
function setupDateRangeFilter(fireData) {
  ...
  const onRangeChange = () => {
    ...
    if (sDate < new Date("2025-04-01") || eDate > new Date("2025-04-30")) {
      alert("2025년 4월 범위 내에서만 선택 가능합니다.");
      return;
    }
