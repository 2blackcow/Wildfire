...
async function init() {
  ...
  // MAXAR 시계열 타일 레이어 (2025-01-09)
  const provider = new Cesium.UrlTemplateImageryProvider({
    url: "https://stormscdn.ngs.noaa.gov/20250109m-maxar/{z}/{x}/{y}",
    tilingScheme: new Cesium.WebMercatorTilingScheme(),
    maximumLevel: 19,
    credit: "NOAA MAXAR 2025-01-09",
  });
  viewer.imageryLayers.addImageryProvider(provider);

  // 🔥 산불 점 로딩
  const dateStr = "2025-01-09";
  const res = await fetch("/fire_archive_SV-C2_616504.json");
  const fireData = await res.json();
  let count = 0;

  fireData.forEach((fire) => {
    if (fire.acq_date !== dateStr) return;
  ...
  console.log(`✅ ${dateStr} 산불 ${count}개 표시 완료`);
